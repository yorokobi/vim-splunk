...
" Group clusters
syn cluster confStanzas contains=confLimitsStanzas,confCommonStanzas,confGenericStanzas

" Stanzas
syn match   confLimitsStanzas contained /\v<(anomalousvalue|associate|authtokens|auto(_summarizer|regress)|concurrency|correlate|ctable)>/
...
syn match   confLimitsStanzas contained /\v<(top|transactions|tscollect|type(ahead|r)|viewstates|xyseries|rollup|dfs)>/
...
syn match   confLimitsStanzas contained /\v<(eval|rest|ingest_actions|spl2|storage_passwords_masking|email|datalake)>/
...
syn match   confLimits /\v<^(check_splunkd_period|chunk_(multiplier|size)|compression_level|concurrency_message_throttle_time)>/
...
syn match   confLimits /\v<^(max(KBps|ReducersPerPhase|_accelerations_per_collection|_action_results|_blocking_secs|_bucket_bytes))>/
...
syn match   confLimits /\v<^(max_(verify_(bucket(_time|s)|ratio|total_time)|workers_searchparser)|foreach)>/
...
syn match   confLimits /\v<^(reuse_map_maxsize|saved_searches_disabled|scheduled_view_timeout|sensitivity)>/
...
syn match   confLimits /\v<^(shc_(accurate_access_counts|(role|syswide)_quota_enforcement)|show_warn_on_filtered_indexes|shp_dispatch_to_member)>/
...
syn match   confLimits /\v<^(status_(buckets|cache_(in_memory_ttl|size)|period_ms)|subsearch_(max(out|time))|summariesonly|summary_mode)>/
syn match   confLimits /\v<^(suppress_derived_info|sync_bundle_replication|target_time_perchunk|tdigest_(k|max_buffer_size))>/
syn match   confLimits /\v<^(threads|threshold_(connection_life_time|data_volume)|time(_format_reject|line_(events_preview|freq)))>/
syn match   confLimits /\v<^(tocsv_(maxretry|retryperiod_ms)|track_indextime_range|truncate_report|ttl|unified_search|use_(bloomfilter|cache|directives|metadata_elimination))>/
...
syn match   confLimits /\v<^(remote_search_requests_throttling_type|async_quota_update(_freq)?|enable_createrss_command)>/
...
" Constants
syn match   confLimitsConstants /\v<(host|splunk_server|consec_not_needed|everything|disabledSavedSearches|DEBUG|INFO|WARN|ERROR|csv|srs|gzip|zstd)$>/
...
syn match   confLimitsConstants /\v<((single|multi)threaded|fixed-width|immediate|ttl|app|user|system)$>/
syn match   confLimitsConstants /\v<(overflow|per_cpu|physical_ram|push|pull|djb2|xxh32)$>/

" Deprecated
syn match   confLimitsDeprecated /\v<^(enable_file_command|use_dispatchtmp_dir|rr_(m(ax|in)_sleep_ms)|rr_sleep_factor)>/
syn match   confLimitsDeprecated /\v<^(subsearch_timeout|tailing_proc_speed|time_before_close|shc_local_quota_check)>/
syn match   confLimitsDeprecated /\v<^(shp_dispatch_to_slave|cmds_black_list|maxRunningPrdSearches)>/

hi def link confLimitsStanzas Identifier
hi def link confLimits Keyword
hi def link confLimitsConstants Constant
hi def link confLimitsDeprecated Removed
