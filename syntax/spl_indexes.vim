...
" Group clusters
syn cluster confStanzas contains=confIndexesStanzas,confIndexesDeprecatedStanzas,confCommonStanzas,confGenericStanzas

" Stanzas
syn match   confIndexesStanzas contained /\v<(volume:[^\]]+)>/
...
syn match   confIndexes /\v<^(processTrackerServiceInterval|quarantine(Future|Past)Secs|raw(Chunk|File)SizeBytes)>/
...
syn match   confIndexes /\v<^(archiver\.selfStorage(Provider|Bucket(Folder)?)|allowBulkDataMove)>/
syn match   confIndexes /\v<^(rotatePeriodInSecs|rtRouter(QueueSize|Threads)|service((InactiveIndexes|Meta|SubtaskTiming)Period))>/
...
syn match   confIndexes /\v<^(timePeriodInSecBeforeTsidxReduction|tsidx(ReductionCheckPeriodInSec|StatsHomePath|SyncPeriod)|tstatsHomePath)>/
...
syn match   confIndexes /\v<^(vix\.splunk\.search\.recordreader\.((csv|sequence)\.regex)|hashTableSize)>/
...
syn match   confIndexes /\v<^(vix\.unified\.search\.cutoff_sec|warmToColdScript|perfTuneMetaData(API|Read|Write)s|quadReloadHashTable)>/
...
syn match   confIndexes /\v<^(fileSystemExecutorWorkers|hotBucketStreaming.extraBucketBuildingCmdlineArgs)>/
...
syn match   confIndexes /\v<^(remote\.s3\.use_sdk|remote\.azure\.(tsidx_compression|federated_token_file|authority_host))>/
...
syn match   confIndexes /\v<^(remote\.azure\.azure_kv\.(federated_token_file|authority_host))>/
syn match   confIndexes /\v<^(mdl(Index|RemotePath)|remote.s3.object_locks_retention_period)>/
...
syn match   confIndexesConstants /\v<(azure-sse-(kv|ms)|cse|(m)?s|azure_kv|mtime|current|max_count)$>/
syn match   confIndexesConstants /\v<(aes-256-gcm|4-only|6-only|ssl3|gcp-sse-(c|kms|gcp)|azure-sse-c)$>/

" Deprecated
syn match   confIndexesDeprecated /\v<^(serviceOnlyAsNeeded|remote\.(azure|s3)\.tsidx_compression|queryLanguageDefinition)>/

hi def link confIndexesStanzas Identifier
hi def link confIndexes Keyword
hi def link confIndexesConstants Constant
hi def link confIndexesDeprecatedStanzas Removed
hi def link confIndexesDeprecated Removed
