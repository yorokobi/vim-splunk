...
" Group clusters
syn cluster confStanzas contains=confServerStanzas,confCommonStanzas,confGenericStanzas

" Stanzas
syn match   confServerStanzas contained /\v<(applicationsManagement|cachemanager|cluster(ing|master:[^]]+))>/
...
syn match   confServerStanzas contained /\v<(data_management|opamp_binary_(splunk_)?edge|etcdTlsConfig|tls1\.3|kvstoreSslClientConfig)>/
syn match   confServerStanzas contained /\v<(scs|spotlight(:splunkd)?|version_control:compression|cluster_state_server|data_orchestrator)>/
syn match   confServerStanzas contained /\v<(heap_profiler)>/
...
syn match   confServer /\v<^(|captain_(is_adhoc_searchhead|uri)|(certCreateScript|checkFrequency|cipherSuite))>/
...
syn match   confServer /\v<^(commit_retry_time|components|conf_deploy_(concerning_file_size|fetch_(mode|url)|repository|staging))>/
...
syn match   confServer /\v<^(conf_replication_summary\.(concerning_file_size|period))>/
...
syn match   confServer /\v<^(description|detailsUrl|dh(F|f)ile|disable(d|DefaultPort)|ecdhCurves|election(_timeout_(ms|2_hb_ratio))?)>/
syn match   confServer /\v<^(embedSecret|enable(S2SHeartbeat|SplunkdSSL)|etc_filesize_limit|eviction_policy|executor_workers)>/
...
syn match   confServer /\v<^(log(_age|_heartbeat_append_entries|inUrl)|long_running_jobs_poll_period|maintenance_mode|manual_detention)>/
...
syn match   confServer /\v<^(multisite|no_(artifact_replications|proxy)|notify_scan_(min_)?period|oplogSize|parallelIngestionPipelines|pass4SymmKey)>/
...
syn match   confServer /\v<^(sampling_interval|scheduling_heuristic|search_(factor|files_retry_timeout)|sid_proxying|site(_mappings|_(replication|search)_factor)?)>/
...
syn match   confServer /\v<^(ssl(AltNameToCheck|CRLPath|ClientSessionPath|CommonNameToCheck|KeysPassword|RootCAPath|ServerSessionTimeout|VerifyServerCert|Versions(ForClient)?))>/
...
syn match   confServer /\v<^(tar_format|target_wait_time|throwOnBucketBuildReadError|trustedIP|update(Host|Path)|upload_proto_host_port|url)>/
...
syn match   confServer /\v<^(legacyCiphers|splunkd_stop_timeout|decommission_node_force_timeout)>/
...
syn match   confServer /\v<^(remote\.s3\.(timeout\.(connect|read|write)|ssl(Versions|(Common|Alt)NameToCheck|RootCAPath)))>/
...
syn match   confServer /\v<^(remote\.s3\.kms\.(ecdhCurves|dhFile)|remote\.s3\.sslVerifyServerCert)>/
...
syn match   confServer /\v<^(roll_and_wait_for_uploads_at_shutdown_secs|crossOriginSharingHeaders|freeze_during_maintenance)>/
...
syn match   confServer /\v<^(rolling_restart_condition|streaming_replication_wait_secs|rebalance_primaries_execution_limit_ms)>/
...
syn match   confServer /\v<^(collectLocalIndexes|deployerPushThreads|storageEngine|clientConnection(Timeout|PoolSize)|clientSocketTimeout)>/
...
syn match   confServer /\v<^(allowWwwAuthHeader|imds_version|manager_uri)>/
...
syn match   confServer /\v<^(kvstoreUpgrade(CheckInterval|OnStartup(Retries|Delay))|provider|allocated_max_threads_percentage)>/
syn match   confServer /\v<^(run_as_owner_enabled|repoDir)>/
syn match   confServer /\v<^(denylist|log_throttling_(disabled|threshold_ms)|exclude_fields)>/
...
syn match   confServer /\v<^(defaultCidrPrefixLength|ocspValidation|pool_size|edge_processor_enabled|otel_collector_management_enabled)>/
syn match   confServer /\v<^(skipSearchSessionTokenInvalidationOnLogout|allowed_unarchive_commands|deprecatedTlsVersionSupport)>/
syn match   confServer /\v<^(insecureAutoGeneratedTls|server(Cert|Key)File|groups|(archivedS|s)plunkbaseAppsDumpUrl)>/
syn match   confServer /\v<^(request_full_license_info|max_peer_truncate_load|hot_bucket_slice_poll_frequency_ms)>/
syn match   confServer /\v<^(clustered_bucket_database_granularity|conf_replication_max_csv_lookup_size_bytes|kvserviceConnectionString)>/
syn match   confServer /\v<^(forceLocalForInternalCollections|shutDownMongo|max_restore_external_attempts|postgresMigrateOnStartup)>/
syn match   confServer /\v<^(clientCert|mvl-enabled|enable_for_sync_transactions|job_state_store|enable_otlp_(traces|metrics(_export)?))>/
syn match   confServer /\v<^(traces_head_sampling_rate|interval|http_port|helper_process_read_timeout_secs|etcd_backup_interval)>/
syn match   confServer /\v<^(max_peerdown_time|etcd_auto_compaction_retention|etcd_defrag_interval|spl2_preview_enabled)>/
syn match   confServer /\v<^(period|output_prefix|min_free_space)>/
...
syn match   confServerConstants /\v<(force_python3|weighted_random|round_robin|up|batch_adding|starting)$>/
...
syn match   confServerConstants /\v<(crl|manual|tcp|unspecified|splunk|OS)$>/
syn match   confServerConstants /\v<(round_robin|weighted_random|blocked_queue_count|AWS|KVstore)$>/
syn match   confServerConstants /\v<(cohosted|local|per_client|global|index|external|kvstore)$>/

" Complex keys
syn match   confComplex /\v<^((EXCLUDE|SEARCHFILTER(LUHN|SIMPLE))-\w+)>/

" Deprecated
syn match   confServerDeprecated /\v<^(supportSSLV3Only|sslCommonNameList|ecdhCurveName|sslKeysfile(Password)?|slaves)>/
syn match   confServerDeprecated /\v<^(sslRootCAPathHonoredOnWindows|ca(CertFile|Path)|cookieSameSiteSecure|master_uri)>/
syn match   confServerDeprecated /\v<^(rebalance_primaries_execution_limit|local_executor_workers|password|rootCA|dhfile)>/
syn match   confServerDeprecated /\v<^(conf_replication_summary\.((black|white)list\.[^\ |\=]+)|compressed|master_dump_service_periods)>/
syn match   confServerDeprecated /\v<^(encrypt_fields|enable_jobs_data_lite|storageEngineMigration|caCertPath)>/
syn match   confServerDeprecated /\v<^(ssl(VerifyServerCert|VerifyServerName|Keys(Path|Password))|kvstoreUpgradeOnStartupEnabled)>/
syn match   confServerDeprecated /\v<^(enable_clustered_mode)>/
...
hi def link confServerDeprecated Removed
